...
void FileLogger::makeBackup()
{
    ...
    QString renameToPattern = m_path.data() + u".bak%1"_qs;
    int counter = 1;
    if (m_compressBackups)
    {
        renameFrom = m_path + u".gz."_qs + QString::number(QDateTime::currentSecsSinceEpoch(), 36);
        QString err;
        ...
        {
            renameToPattern = m_path.data() + u".%1.gz"_qs;
            Utils::Fs::removeFile(m_path);
        }
    }
...
{
    const QStringList nameFilter { u"qbittorrent.log"_qs + (m_compressBackups ? u".*.gz"_qs : u".bak*"_qs) };
